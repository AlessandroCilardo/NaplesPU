...
typedef struct IMMENSE { unsigned int l, r; } immense;
typedef struct GREAT { unsigned int l, c, r; } great;
...
unsigned int getbit(immense source, int bitno, int nbits);
void ks(/*immense key, */int n, great * kn);
void cyfun(unsigned int ir, great k, unsigned int * iout);
...
void cyfun(unsigned int ir, great k, unsigned int * iout) {
   ...
   great ie;
   unsigned int itmp,ietmp1,ietmp2;
   char iec[9];
   ...
   ie.c ^= k.c;
   ie.l ^= k.l;
   ietmp1=((unsigned int) ie.c << 16)+(unsigned int) ie.r;
   ietmp2=((unsigned int) ie.l << 8)+((unsigned int) ie.c >> 8);
   for (j=1,m=5;j<=4;j++,m++) {
      iec[j]=ietmp1 & 0x3fL;
      iec[m]=ietmp2 & 0x3fL;
      ietmp1 >>= 6;
